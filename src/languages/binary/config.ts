  ...
  if (lessonId === 'overview') {
    ...
    return [
      answer(`binary-${lessonId}-${seed}-states`, 'create', '状态数量', `${bits} 个 bit 一共有多少种不同状态？写出计算式和结果。`, `计算式：2^${bits} = `, `2^${bits} = ${2 ** bits}`, [`每个 bit 有 2 种状态。`, `把 2 连乘 ${bits} 次。`, `状态数量是 ${2 ** bits}。`], (source) => {
        const normalized = compact(source).replace(/[＝]/g, '=').replace(/[×x]/gi, '*').replace(/\*\*/g, '^').replace(/⁰/g, '0').replace(/¹/g, '1').replace(/²/g, '2').replace(/³/g, '3').replace(/⁴/g, '4').replace(/⁵/g, '5').replace(/⁶/g, '6')
        const powerForm = new RegExp(`2\\s*\\^\\s*${bits}\\s*=\\s*${2 ** bits}`).test(normalized)
        const repeatedMultiplication = new RegExp(`^2(?:\\*2){${bits - 1}}=${2 ** bits}$`).test(normalized)
        return powerForm || repeatedMultiplication
