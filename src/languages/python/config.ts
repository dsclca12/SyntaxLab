...
- **把 .venv 提交到 Git**：不需要。把 **.venv/** 写入 **.gitignore**；别人用同一份依赖清单重新创建即可。` },
    ...
    lesson('operators', '05', '运算符逐个讲解', '不把符号挤成一张速查表：逐个理解结果、适用场景、优先级和常见错误。', [
      { heading: '`+` 加法与拼接', body: '`3 + 2` 得到 `5`；`"Py" + "thon"` 得到 `"Python"`；列表也能用 `+` 连接。两侧类型必须兼容，`"年龄：" + 18` 会报 TypeError，应先写 `str(18)` 或使用 f-string。`+value` 是一元正号，通常只用于强调数值符号。' },
      { heading: '`-` 减法与取负', body: '`10 - 3` 得到 `7`；`-amount` 会把数值变为相反数。减法不能直接用于字符串或列表。连续符号要按一元运算理解：`5 - -2` 等于 `5 - (-2)`，结果是 7，但实际代码最好加括号。' },
      { heading: '`*` 乘法与重复', body: '`6 * 7` 得到 `42`。字符串或列表乘整数表示重复：`"ha" * 3` 得到 `"hahaha"`，`[0] * 3` 得到 `[0, 0, 0]`。创建嵌套列表时不要写 `[[0] * 3] * 2`，因为两行会引用同一个内层列表；应使用列表推导式。' },
      { heading: '`/` 普通除法', body: '`7 / 2` 得到浮点数 `3.5`；即使整除，`6 / 3` 也得到 `2.0`。除数为 0 会触发 ZeroDivisionError。金额分摊时还要考虑舍入规则，不能只依赖二进制浮点数。' },
      { heading: '`//` 向下取整除法', body: '`7 // 2` 得到 `3`。它不是简单“删除小数部分”，而是向负无穷取整，所以 `-7 // 2` 得到 `-4`，不是 `-3`。分页、整组计数前先确认你需要的是向下取整还是截断。' },
      { heading: '`%` 取模（余数）', body: '`17 % 5` 得到 `2`，常用于奇偶判断、循环索引和整除检查：`number % 2 == 0` 表示偶数。Python 的余数与除数同号，因此 `-7 % 3` 得到 `2`；并且始终满足 `a == (a // b) * b + (a % b)`。' },
      { heading: '`**` 幂运算', body: '`2 ** 3` 得到 `8`，表示 2 的 3 次方；平方根可写 `number ** 0.5`。幂运算从右向左结合：`2 ** 3 ** 2` 等于 `2 ** (3 ** 2)`。它比一元负号优先，所以 `-3 ** 2` 得到 `-9`，要计算正 9 应写 `(-3) ** 2`。' },
      { heading: '`@` 矩阵乘法', body: '`@` 为矩阵乘法协议预留，普通 int、float、list 不支持它。NumPy 等科学计算库中的数组会实现该运算，例如 `matrix_a @ matrix_b`。基础程序不要把 `@` 当作普通乘号；普通乘法仍使用 `*`。' },
      { heading: '`==` 与 `!=`：相等和不等', body: '`left == right` 比较值是否相等，`left != right` 比较是否不等，结果都是 bool。`=` 是赋值，不能当成比较符。浮点计算可能存在微小误差，像 `0.1 + 0.2 == 0.3` 可能为 False，需要容差比较。' },
      { heading: '`>`、`>=`、`<`、`<=`：大小比较', body: '`>` 大于，`>=` 大于等于，`<` 小于，`<=` 小于等于。边界是否包含决定有没有等号。Python 支持连续比较：`18 <= age <= 60` 等价于 `age >= 18 and age <= 60`，中间表达式只求值一次。' },
      { heading: '`and`：两边都要成立', body: '`condition_a and condition_b` 在左边为假时不会计算右边，这叫短路。它返回的是参与运算的对象，不一定是 bool：`"" and 5` 返回空字符串。用于条件时按真假理解；需要严格布尔值时使用 `bool(...)`。' },
      { heading: '`or`：至少一边成立', body: '`condition_a or condition_b` 在左边为真时不会计算右边。`name = input_name or "匿名"` 可提供默认值，但 0、空容器等合法假值也会被替换；只想处理 None 时，应显式写 `if value is None`。' },
      { heading: '`not`：反转真假', body: '`not value` 一定返回 bool。`not True` 是 False，`not []` 是 True。`not` 的优先级低于比较运算，所以 `not score >= 60` 等价于 `not (score >= 60)`；通常直接写 `score < 60` 更易读。' },
      { heading: '`in` 与 `not in`：成员判断', body: '`item in container` 检查是否包含，`item not in container` 检查是否不包含。字符串检查子串，列表和集合检查元素；字典默认检查键：`"name" in student`，检查值要写 `value in student.values()`。' },
      { heading: '`is` 与 `is not`：身份判断', body: '`is` 判断两个名字是否指向同一个对象，不是判断值是否相等。规范场景是 `value is None` 与 `value is not None`。不要写 `text is "ok"` 或 `number is 1`，这些结果可能受对象缓存影响；值比较应使用 `==`。' },
      { heading: '`&`、`|`、`^`：按位与、或、异或', body: '对整数，它们逐位处理二进制：`6 & 3` 得到 2，`6 | 3` 得到 7，`6 ^ 3` 得到 5。对集合，`&`、`|`、`^` 分别表示交集、并集、对称差。不要用 `&`、`|` 代替逻辑 `and`、`or`，它们的优先级和求值方式不同。' },
      { heading: '`~`、`<<`、`>>`：取反与移位', body: '`~number` 对整数逐位取反，满足 `~n == -n - 1`。`value << count` 左移，非负整数通常相当于乘以 `2 ** count`；`value >> count` 右移，通常相当于向下除以相应的 2 次幂。常见于权限位、颜色和二进制协议。' },
      { heading: '`=` 与增强赋值', body: '`=` 把右侧结果绑定给左侧名字。`+=`、`-=`、`*=`、`/=`、`//=`、`%=`、`**=` 以及位运算对应形式会在原值基础上更新，例如 `count += 1`。对列表，`items += more` 会原地扩展；`items = items + more` 会创建新列表，这个差别会影响其他引用。' },
      { heading: '`:=` 海象运算符', body: '`:=` 在表达式内部赋值，例如 `while (line := file.readline()):`。它适合避免重复计算，但会让“计算”和“赋值”挤在一起；初学阶段先写成两行更清楚。使用时通常需要括号，而且不要为了少一行而滥用。' },
      { heading: '优先级与括号', body: '常见顺序可记为：`**` → 一元 `+ - ~` → `* / // % @` → `+ -` → 移位 → 位运算 → 比较、`in`、`is` → `not` → `and` → `or`。不必死背整张表；混合逻辑时用括号表达业务意图，例如 `(is_member or amount >= 200) and not is_blocked`。' },
    ], 'a = 17\nb = 5\n\nprint(a + b)   # 22\nprint(a - b)   # 12\nprint(a * b)   # 85\nprint(a / b)   # 3.4\nprint(a // b)  # 3\nprint(a % b)   # 2\nprint(a ** 2)  # 289\n\neligible = (18 <= a <= 60) and a not in {20, 30}\nprint(eligible)', ['能逐个解释常用算术运算符及负数边界', '能区分值比较、身份判断和成员判断', '能正确选择逻辑运算与位运算', '能解释增强赋值是否会修改原对象', '能用括号写出没有歧义的复合表达式'], [
      ...
      ex('operators-3', '区分身份和值', '只在 result 没有值时输出“未找到”。', '', 'if result is None:\n    print("未找到")', ['None 使用身份判断。', '运算符是 is。', '不要写 == None。'], has(/result\s+is\s+None/)),
      ex('operators-4', '组合优惠条件', '会员或金额不少于 200，并且不在黑名单时，将 can_discount 设为 True。', '', 'can_discount = (is_member or amount >= 200) and not is_blocked', ['先用 or 组合两种资格。', '用括号包住资格条件。', '最后用 and not 排除黑名单。'], has(/\(\s*is_member\s+or\s+amount\s*>=\s*200\s*\)\s+and\s+not\s+is_blocked/)),
      ex('operators-5', '拆出颜色通道', '使用右移和按位与，从整数 color 中取出最低 8 位并保存到 blue。', '', 'blue = color & 0xFF', ['最低 8 位不需要右移。', '掩码 0xFF 的 8 位都是 1。', '按位与使用 &。'], has(/blue\s*=\s*color\s*&\s*0x[fF]{2}/)),
    ], '碰到陌生运算符时，不只查“结果是什么”，还要测试类型、负数、0、空值和是否原地修改。'),
